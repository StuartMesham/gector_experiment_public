...
from helpers_new import write_lines, read_parallel_lines, encode_verb_form, \
...
def apply_transformation(source_token, target_token):
    ...
    if len(target_tokens) > 1:
        ...
        if transform:
            return transform
    checks = [check_equal, check_casetype]
    for check in checks:
...
def align_sequences(source_sent, target_sent):
    ...
    sent_with_tags = json.dumps({'tokens': ['$START'] + source_tokens, 'labels': labels})
...
def convert_tagged_line(line):
    json_line = json.loads(line)
    source_tokens = json_line['tokens'][1:]
